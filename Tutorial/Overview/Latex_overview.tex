...
\indent report(論文or報告)：新的一章會自動從新的一頁開始。
...
使用下面的方法可以直接進入隨文數學模式：\\
\verb|$ 數學式 $|：就這就是前面小節所使用的方法，這個方法通常用來插入簡單的數學式 $f(x)$ 或符號 $F=ma$ 。在這種模式下數學式就會隨著內文一起呈現。
		\begin{framed}
			\begin{verbatim}
...
\subsection{矩陣}
沒有外括號的矩陣可以透過 \verb|\begin{array} 矩陣內容 \end{array}| 來達成，矩陣內部的寫法64都是使用 \& 將各個欄位分開。
	\begin{equation}
		\begin{array}{ccc}
		a & b & c \\
		d & e& f \\
...
若我們並不需要方程式的子標號，這時候只要在 \verb|align| 環境中再使用 \verb|split| 環境包起來即可。
\begin{align}
\begin{split}
f(x) =\quad & x^2-(a+b)x+ab\\
	=\quad & x^2-ax-bx+ab\\
...
\begin{enumerate}
	\item 試打出下列數學式\\
	\begin{equation}
	 f(x,\mu,\sigma) = \frac{1}{\sigma \sqrt{2\pi}}(\frac{x-\mu}{\sigma})^2
	\end{equation}
...
\item \textbf{縱線、項目對齊方式}：表格的縱線在表格插入時(第五行)就設定好了：需要幾個比較項目、表格中每個項目的對齊方式、項目之間需要幾條縱線也都是在這裡設定。
\item \textbf{橫線}：表格的橫線則是使用 \verb|\hline| 指令插入，用幾次指令就可以得到幾條橫線。
